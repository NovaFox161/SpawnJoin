...
public class Lobbies implements CommandExecutor {
    private Main plugin;
        ...
        if (cmd.getName().equalsIgnoreCase("lobbies")) {
            if (plugin.getConfig().getString("Commands.Lobby.Enabled").equalsIgnoreCase("True")) {
                if (!(sender.hasPermission("SpawnJoin.use.lobbies"))) {
                    ...
                    if (FileManager.getListYml().contains("Lobbies")) {
                        if (!(sender instanceof Player)) {
                            String original = FileManager.getListYml().getString("Lobbies");
                            String replaced = original.replace("[", "").replace("]", "");
                            for (String lobby : FileManager.getListYml().getStringList("Lobbies")) {
                        ...
                        } else if (plugin.getConfig().getString("Inventory.Lobby.Use").equalsIgnoreCase("True")
